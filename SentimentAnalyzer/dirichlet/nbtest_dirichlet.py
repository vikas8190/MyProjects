...
''' Function to calculate probability from model file'''

def read_model_probs(class_doc_stats,class_prob,word_dict,model_file):
	f=open(model_file,"r")
...
''' Function to predict class for test and dev data'''

def predict_class(prediction,class_doc_stats,class_prob,word_list,word_dict):
	prob_values=[]
	new_prob_values=[]
...
''' Function to predict class for test and dev data'''
def dir_prediction(prediction,dir_name,class_doc_stats,class_prob,word_dict):
	test_pred={}
	for filename in os.listdir(dir_name):
		...
		pred_prob,predicted_class=predict_class(prediction,class_doc_stats,class_prob,word_list,word_dict)
...
'''Function to generate predictions for test and dev data'''
...
def main(argv):
	...
	if argv:
		...
		test_dir=argv[1]
		pred_file=argv[2]
	read_model_probs(class_doc_stats,class_prob,word_dict,model_file)
	f=open(pred_file,"w")
	f.writelines("##########################################################################################\n")
	f.writelines("Prediction for given Directory:"+str(test_dir)+"\n")
	f.writelines("##########################################################################################\n")
	prediction,pred_values=dir_prediction(prediction,test_dir,class_doc_stats,class_prob,word_dict)
